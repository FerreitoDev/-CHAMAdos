...
export const DeactivateCategoryDialog: React.FC<DeactivateCategoryDialogProps> = ({
  isOpen,
  category,
  onClose,
  onConfirm,
  ...
  const handleConfirm = async () => {
    ...
    try {
      ...
      if (err instanceof Error) {
        ...
        setApiError('Erro ao desativar categoria. Tente novamente.')
      }
    } finally {
      setIsSubmitting(false)
    }
  ...
  return (
    <div
      ...
      data-testid="deactivate-category-dialog"
    >
      <div className="relative w-full max-w-md bg-card border border-border rounded-xl shadow-2xl p-6 text-foreground overflow-hidden">
        ...
        <div className="space-y-3 mb-6">
          ...
          <div className="p-3 rounded-lg bg-amber-500/10 border border-amber-500/20 text-amber-500 text-xs">
            ⚠️ Esta ação deixará a categoria indisponível para seleção em novos chamados.
            O histórico de chamados existentes que utilizam esta categoria será preservado.
          </div>
        </div>
